...
Napi::Value RbdImageWrap::MetadataGet(const Napi::CallbackInfo& info) { auto key = string_arg(info, 0, "key"); auto image = image_; return async_value<std::string>(info.Env(), "rbd_metadata_get", name_ + "/" + key, [image, key](std::string& out) { size_t size = 256; int rc; do { std::vector<char> data(size); size_t actual = size; rc = image->use([&](rbd_image_t h) { return rbd_metadata_get(h, key.c_str(), data.data(), &actual); }); if (rc >= 0) { out.assign(data.data(), actual ? actual - 1 : 0); return 0; } size = actual > size ? actual : size * 2; } while (rc == -ERANGE); return rc; }, [](Napi::Env env, std::string& out) { return Napi::String::New(env, out); }); }
